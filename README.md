...
> **v6 进行中（2.5D 立体化）**：引擎已加入格子高度（z），并新增独立的等轴 **2.5D 渲染器**（纯 CSS transforms，无 Three.js），把高低差/台阶/推下/折跃升台真实表现出来——旧 52 关仍走原 2D 网格、行为不变。目前 3D 仅有一个开发用「立体演示」关（菜单底部 dev 入口）；正式 3D 章节与机制（斜坡/升降/桥/竖直折跃/双端受力）尚未上线。另已补「继续上次 / 下一关」体验闭环。详见 `claude.md` §9。
