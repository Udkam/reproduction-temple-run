...
- Independent diagnostic visual reviewer starts only after the one evaluator exits. It may write only `docs/workstreams/temple-tr4-visual-review/TR4_DIAGNOSTIC_REVIEW.md` and `docs/workstreams/temple-tr4-visual-review/THREAD_LOG.md`; all asset, tool, contract, runtime, and evidence images are read-only. It inspects the four beauty frames and their bound masks/depth, and cannot repair or rerender.
