...
Status: **C7 SECOND DRY PREFLIGHT PRECHECK_BLOCKED; EXACT C7-T2G AUTHORIZATION-TOKEN CORRECTION AUTHORIZED; DIAGNOSTIC PROCESS BLOCKED.**
...
C7-T2F completed at `de625c6` and passed independent `SOURCE_READY`. Its one fresh non-writing dry preflight returned `PRECHECK_BLOCKED` with exact reason `diagnostic-007 authorization status is not the frozen C7 source authorization`; `diagnostic-007` remained absent and neither Blender nor evaluator launched. The runner still searched for the superseded C7-D4 top-status sentence even though this authorization's active top status had correctly advanced to C7-T2F before the run. C7-T2G may modify exactly `tools/temple-asset-pipeline/run_tide_scar_tr4_pack.py`, replacing only that obsolete authorization string literal with the durable token `TEMPLE-TR4-C7-SOURCE-CHAIN-AUTHORIZED` above. It must not change the token text, authorization path, any other authorization check, construction/schema/value/hash logic, output, subprocess ceiling, or no-retry behavior. UTF-8 AST, diff check, current-document positive validation, token-removal negative validation, independent source review, and a bounded runner-only commit are required before one newly authorized non-writing dry preflight. Blender remains blocked until that result is `READY_FOR_BLENDER`.
...
12. **C7-T2G authorization-token correction:** exact runner path and one string-literal replacement only, followed by static positive/negative checks, independent review, and a separate source commit.
13. **Newly authorized dry preflight:** only after C7-T2G is committed and all three script hashes are stable. It must stop before Blender on any mismatch.
14. **C7-E1 diagnostic evidence:** one Blender 4.5.5 background process, exactly 20 PNG writes, then at most one evaluator. New `diagnostic-007/**` plus its exact ignored `blender.log` form a separate evidence commit. No retry.
15. **C7-V1 independent visual review:** whenever the runner stops with the exact complete 20-PNG set, regardless of evaluator invocation/result, perform docs-only `TR4_DIAGNOSTIC_REVIEW.md` and visual-review `THREAD_LOG.md`; no repair or rerender. An incomplete set is not manually reviewed.
