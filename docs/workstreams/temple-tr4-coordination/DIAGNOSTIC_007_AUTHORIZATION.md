...
Status: **DIAGNOSTIC-007 IMMUTABLE DIAGNOSTIC_BLOCKED BEFORE RENDER; ZERO PNG; NO RETRY.**

C7 launch authorization token: `TEMPLE-TR4-C7-SOURCE-CHAIN-AUTHORIZED`.
...
C7-T2F completed at `de625c6` and passed independent `SOURCE_READY`. Its one fresh non-writing dry preflight returned `PRECHECK_BLOCKED` with exact reason `diagnostic-007 authorization status is not the frozen C7 source authorization`; `diagnostic-007` remained absent and neither Blender nor evaluator launched. The runner still searched for the superseded C7-D4 top-status sentence even though this authorization's active top status had correctly advanced to C7-T2F before the run. Independent review of `efc0b3f` found two P1 gaps: runner's next check still required an absent historical CURRENT_TASK phrase, and matching a bare token substring would also accept explanatory prose if the authoritative line were removed. Corrected contract `9557be3` replaces that draft with `splitlines().count(...) == 1` semantics for the complete labelled authorization line above and the complete labelled task-authorization line in `docs/CURRENT_TASK.md`; fresh independent review found no P0/P1/P2 and returned `READY_FOR_C7_T2G_CONTRACT`. C7-T2G now authorizes exactly `tools/temple-asset-pipeline/run_tide_scar_tr4_pack.py` and only those two obsolete predicate replacements. It must not change either labelled line, either document path, the independent-verdict or coordination-log checks, construction/schema/value/hash logic, output, subprocess ceiling, or no-retry behavior. UTF-8 AST, diff check, a current-document positive validation, separate deletion and duplication negatives for each labelled line, independent source review, and a bounded runner-only commit are required before one newly authorized non-writing dry preflight. Blender remains blocked until the later dry result is `READY_FOR_BLENDER`.

C7-T2G completed at `973d2fa` with runner SHA-256 `ef99b614f2095cc82445780a39add825d42c239afa5d766302fd7672a00e5d58` and independent `SOURCE_READY`. The one newly authorized command `python tools/temple-asset-pipeline/run_tide_scar_tr4_pack.py --dry-preflight` then ran at HEAD `cee5b12`, returned exit `0`, and emitted only `READY_FOR_BLENDER`. `diagnostic-007` remained absent; Blender and evaluator were not launched. The source hashes and authorization inputs are now stable for exactly one standard runner invocation. That invocation may start exactly one Blender 4.5.5 process, write the closed 20-PNG set, and invoke at most one evaluator. It has no retry, and its maximum machine verdict remains `READY_FOR_MANUAL_REVIEW`.

The sole standard invocation then ran once and returned exit `1` with `DIAGNOSTIC_BLOCKED Blender exited 1`. Blender 4.5.5 failed before the first render in `actual_material_graph_record` while unconditionally reading `Voronoi.Smoothness`; the live F1/Euclidean node input collection did not contain that dynamic socket. Evaluator return code is null and PNG count is zero. Evidence commit `3937ef6` freezes the exact four-file directory: `preflight.json` 24,054 bytes SHA-256 `4086c7394c54cf6b8ac9f41e4512a74113232547540035f288d55672b7caaf35`, `planned-manifest.json` 3,791 bytes SHA-256 `fe43f335001de5441df71ff43e8c3c2dae9451f238d6e50a0a347204b1d229dc`, `blender.log` 1,774 bytes SHA-256 `70ed1b44c0bf46edb99d7b41a7334ec24d6bab68ae8459bfd8de895b166f2177`, and `diagnostic-status.json` 363 bytes SHA-256 `029549ce684c45d93d42524b3aaf2bc173adb102b9a55d23750a9244d1f5373f`. Diagnostic `007` is terminal and cannot be repaired, rerun, evaluated, or visually reviewed. Any new material-socket probe or diagnostic must use a new identity and fresh independent contract.
...
11. **Second non-writing dry preflight:** consumed `PRECHECK_BLOCKED` before Blender because runner required the superseded C7-D4 top-status sentence; `diagnostic-007` remained absent.
12. **C7-T2G corrected contract review:** independent read-only review of the two complete labelled line/count predicates. Source remains blocked until a separate coordinator reauthorization.
13. **C7-T2G authorization-token correction after reauthorization:** exact runner path and only the two obsolete predicate replacements, followed by static deletion/duplication negatives, independent review, and a separate source commit.
14. **Newly authorized dry preflight:** complete at HEAD `cee5b12`; exit `0`, sole output `READY_FOR_BLENDER`, and `diagnostic-007` absent.
15. **C7-E1 diagnostic evidence:** terminal `DIAGNOSTIC_BLOCKED` at stage `blender`; exactly four files, zero PNG, evaluator not invoked, evidence commit `3937ef6`, and no retry.
16. **C7-V1 independent visual review:** not triggered because the render set is incomplete. No visual claim may be made from C7.
