...
The R1D silhouette layer uses closed terraced rock bodies rather than repeated prisms: separate shelf elevations, broken returns, non-mirrored notches, and limited overhangs preserve genuine negative space without adding a continuous wall. R1E keeps that massing and makes it readable through one continuous clean-room basalt surface language. World-scaled UVs and face-aware values distinguish upward caps and shelves from risers, notch returns, and undersides; texture/value contrast is strongest near the camera, restrained in the middle distance, and quietest at the horizon. Extreme cavity readability may use only a bounded face-scoped additive shift of the scene's existing hemisphere sky/ground mixing weight before native AO and PBR output: top/shelf/wall stay unchanged, return receives at most `.10`, underside at most `.15`, and no correction exists without a HemisphereLight. Candidate-bound ray attribution may reopen only the existing horizon-island vertex positions when it proves geometry, rather than material darkness, causes the defect. V6 makes near notches shallower than mid and far notches while preserving topology/counts and asymmetric genuine gaps, but its blocked frames prove equal motion of the upper and intermediate rings leaves the upper riser downward-facing. V7 therefore changes only the near/mid intermediate-ring radius so its final, post-ledge effective radial scale is exactly `.04` beyond the corresponding upper ring; implementation compensates the frozen overhang multipliers (`1.14` upper, `.92` intermediate) rather than changing them. The small outward bridge must keep every actual upper-riser triangle below the `.55` upward-plane threshold while removing every negative normal. Near/mid upper-minus-intermediate radial deltas must stay outward and bounded between `-1m` and `-.2m`, replacing only their obsolete positive upper-overhang gate; far components retain the positive `>.35m` gate. Only the adjacent near/mid lower shelf may narrow to an outer/inner area ratio above `1.23`; upper shelves and far-band shelves retain the prior `>1.25` massing gate, with far geometry and all heights frozen. It cannot alter direct/specular light, shadows, emissive, exposure, post-light output, or authored vertex normals. Surface treatment must not become a checker, noisy displacement illusion, stretched seam, black wall, glowing recess, flat gray panel, repeated mouth row, continuous roof, or substitute for geometry.

Accepted R1F design outcome: after the independently accepted R1E-V7 canyon candidate, R1F keeps canonical travel and collision flat and deterministic while rebuilding the visible causeway skin from six continuous instanced sandstone signatures. Each module uses connected, non-coplanar medium-scale top panels, shallow staggered joints that never read as gameplay gaps, and asymmetric broken lips/returns grounded into real side mass. A world-anchored two-scale sandstone response separates sunlit tops, worn edges, and recessed faces without emissive lift, repeated tile stamps, or texture swim. Module terminals remain continuous, the playable corridor cannot acquire a false hole or safe lane, and presentation geometry cannot obscure a beam, ring, column, gap, turn, or runner pose. Candidate `6680489cf2e3ec035da44fc1dde9aa7606bdd9c7` is accepted only for this causeway presentation claim; the primitive courier, pursuer, hazards, final character materials, animation, and broader art direction remain unfinished.

Accepted R1G courier outcome: the centered gameplay character is an original Tide courier rather than a robot assembled from balls, capsules, boxes, and a white target. A tapered cloth torso, layered hood/cowl and shoulder mantle establish one continuous human trunk; sleeved arms, small hands, articulated protected legs, directional boots, a waist harness, a compact inset relay, and two asymmetric closed coat tails add readable medium structure without changing scale or camera. Cloth, leather, porcelain skin, mineral hardware, and the bounded signal inset differ by roughness/metalness/emissive response, but silhouette carries identity first. At 84-91 px landscape height the elbow, knee, separate feet, and forward direction remain legible; portrait adds arm/trunk negative space, waist and forked coat; desktop resolves the hood, mantle, harness/relay, layered torso, legs, and boots. All run phases plus jump, slide, and dead keep connected joints, grounded contact, coat clearance, and distinct action silhouettes. Courier checkpoint `f08a170` plus R1G-E1 candidate `8de73c1` and its 14-frame manifest are independently source, evidence, and visual `READY`. R1G does not authorize or accept a stumble input, new asset, Blender model, camera change, character enlargement, new scene object, pursuer reconstruction, hazard polish, broader animation, or final-game art.

R1G treats `31` total draw calls as the exact ordinary-running composition baseline: the eight desktop gait records (phase 0 also serves as desktop ordinary running), portrait/landscape running records, and portrait reduced-motion record each remain at `31`, for 11 ordinary-composition records total. Fixed semantic records retain their required visible scene composition rather than suppressing hazards or the lifecycle-required pursuer: portrait jump apex is exactly `36`, landscape slide exactly `37`, and desktop game-over exactly `34` total calls; every R1G frame remains at or below `28,000` whole-frame triangles. The complete matrix is bound to seed `1414087749`: gait ticks `60/64/68/72/76/80/84/88`, portrait/landscape/reduced-motion tick `60`, jump tick `21`, slide tick `15`, and game-over tick `336`. These are fixed-record composite baselines, not permission to remove, simplify, or hide required world entities.

Active R1H canyon direction: preserve the accepted four-mesh canyon topology and basalt response while rebuilding the position of its existing vertices into a more credible non-repeating macro silhouette. Near and middle shelf runs need decisive recess, buttress, crown, and shoulder breaks instead of long equal-height extrusions; the existing near/mid/far horizon islands need irregular but bounded crown relief and asymmetric overlap so portrait, desktop, and landscape each show foreground framing, playable middle depth, and an independent far skyline. Geometry must organize the side void through real depth and negative space rather than filling it with a continuous wall. The road, Tide Scar, runner, hazards, pursuer, camera, sky, fog, lights, shader, texture response, and canonical simulation remain unchanged.

R1H is successful only when same-view comparison against candidate `8de73c1` shows a real contour and occlusion change across all three viewports. More noise, subdivisions, contrast, tint, fog, exposure, or texture detail on the same block silhouette is not progress. The four existing mesh/material owners, every index/count, twelve eight-station/six-tier horizon islands, route-safe corridor, genuine gaps, fixed draw-call/triangle matrix, and deterministic world-cell placement remain the hard boundary; no visual gain may come from new scene objects or hiding gameplay.
