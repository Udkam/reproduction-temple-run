...
The C3 wrapped-RAF pass is evidence-`BLOCKED`. Directory `C:\Users\Alex Chen\AppData\Local\Temp\tide-relay-r1h-c3-candidate-de636bc-20260810T002256684\` preserves script SHA-256 `38EE8EA5E82DE4A1367FF86735F6467B1BC4EC5E8673DA3B0403A4A2614D72FE`, empty stdout, and stderr SHA-256 `C87EC9E8842D988E340D7F4EEBF022E2B442DA20BB724F50AB0D33B87D7F5048`; there are zero PNGs and no manifest. The first record timed out after 3,000 ms with canonical hash `fa4e69d7` / tick `60` frozen and tracker intact at `held=false`, `requests=62`, `callbacksStarted=60`, `pending=2`. Camera ended at the unchanged target `z=4.582277164981484`, so the wrapper-covered product/waiter cadence never produced the transient `7.308502044360709 ± .05m` R1G window. Exit was `1`; port `4210` closed. C3 cannot be retried or rewritten.
...
R1H-C4 reopens exactly `GameRuntime.ts` plus its direct test for one DEV-only presentation-hold seam. The method latches before cancel, requests cancellation and clears the pending runtime `frameHandle`, and prevents an already-selected `onFrame` or later `scheduleFrame()` from rendering/rescheduling. It changes no camera, renderer, canonical state, timestamp, or delta. Focused fake-clock verification is followed, after the last source edit, by one final typecheck, complete Vitest suite, and no-delete build. The later sole browser pass restores C1's lightweight unwrapped native RAF waiter; only its matching predicate reads full canonical proof and synchronously calls `qa.holdPresentation()` before screenshot.
