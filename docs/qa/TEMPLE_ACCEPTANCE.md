...
- R1 canyon geometry uses closed near/mid/far components with genuine gaps, outward/non-degenerate topology, multi-level terraces, broken ends, non-mirrored notch profiles, and bounded overhangs. R1E keeps the `ec00e44` position/index fingerprints and exact four-mesh/single-material/zero-group topology fixed while applying the tracked clean-room basalt map to all three bands. Actual indexed-triangle normals classify upward at `normalY>.55`, underside at `<-.55`, and vertical/return otherwise; dominant-plane UV scales are near `.080-.100`, mid `.060-.080`, and far `.045-.060 UV/m`, strictly decreasing with near/far `<2.2`. Mean linear vertex-colour luminance preserves `top > shelf > vertical/return > underside` per actual component; normalized contrast and within-category variation are strictly near `>` mid `>` far, with far contrast `>.12`. The PBR material uses zero emissive, strictly decreasing map strength with adjacent differences `>=.15`, and a strictly increasing non-emissive albedo floor with adjacent differences `>=.03` and ceiling `.25`. R1E-V5 may add one deterministic face attribute that shifts only the existing HemisphereLight sky/ground weight: top/shelf/wall `0`, return `+.10`, underside `+.15`, hard shift cap `.15`, and effective weight `min(1, baseWeight + shift)`. The injection stays before native light maps/end and AO, has exactly zero effect with no HemisphereLight, and makes no assignments to normals, direct/specular lighting, shadows, outgoing light, emissive, exposure, or post-AO colour. The V5 program cache key, actual attributes, deterministic replay, exact numeric shifts, and forbidden writes are test-bound. Browser output remains exactly `31` draw calls / `27,896` triangles. Black crushing, flat slabs, glowing or gray-panel cavities, stretched seams, shimmer, or loss of route readability fails regardless of numeric results.
