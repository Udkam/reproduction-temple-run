...
Result: **TEMPLE-TR4 in progress; material-socket-probe-001 contract review requested, no source/process authority and no visual acceptance.** Diagnostic `007` remains immutable `DIAGNOSTIC_BLOCKED` at evidence commit `3937ef6`: Blender failed before render on missing dynamic `Voronoi.Smoothness`, evaluator did not run, and zero PNGs exist. Independent root-cause review found `Voronoi.Exponent` and guarded Principled inputs unobserved, so diagnostic `008` cannot be authorized from assumptions. The proposed zero-render probe freezes one Blender 4.5.5 process, exact node/property/25-target input, full ordered socket evidence, four-file closure, zero renderer/evaluator/export, and maximum verdict `SOCKET_SCHEMA_OBSERVED`. It requires independent contract readiness and separate coordinator authority; it cannot satisfy any visual gate. TR2 remains the accepted simulation/runtime baseline described below. TR3/A3-A6 and C1-C6 are historical, unintegrated or blocked visual attempts and do not supply TR4 acceptance evidence.
