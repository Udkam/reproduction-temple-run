...
describe('Tide Scar geometric canyon presentation', () => {
  it('builds thick, distinct near/mid/far topology without panorama, plane, or instanced-card geometry', () => {
    ...
    const actual = components.map((component) => {
      ...
      const upperFactors = recoverFactors(shelves[0]!.outer.boundary, upperX, upperZ, upperOuter), lowerFactors = recoverFactors(shelves[1]!.outer.boundary, lowerX, lowerZ, lowerOuter);
      ...
      return { points, center, bounds, profile, seed: spec.seed };
    }).sort((a, b) => b.center.z - a.center.z || a.center.x - b.center.x);
    expect(new Set(actual.map((island) => island.profile)).size).toBe(12); expect(new Set(actual.map((island) => island.seed)).size).toBe(HORIZON_GEOMETRY_CASES.length); for (let island = 1; island < actual.length; island += 1) expect(actual[island]!.profile).not.toBe(actual[island - 1]!.profile);
  ...
  it('preserves R1D massing while actual R1E faces carry deterministic depth-scaled basalt treatment', async () => {
    ...
    expect(fingerprints).toEqual([
      ...
      ['tide-scar-far-low-ridge-mesa-chains', 'b8340206976959896dbdac076e121906a48b0b90a003fe3f6b2ec3484dbe120d', 4392, 4656, [-63.92366027832031, -17.7822322845459, -192], [63.154701232910156, 6.28000020980835, -37.97065734863281]],
    ]);
    ...
    expect({ farCombined: fingerprints[3]![1], rawPositionFingerprints, farIndexFingerprint }).toEqual({
      farCombined: 'b8340206976959896dbdac076e121906a48b0b90a003fe3f6b2ec3484dbe120d',
      rawPositionFingerprints: ['7bcce39ce56b41f6a43673cae1aff35e062bacfc0449adeca09a17a2db60b914', '4f4a56e16b9f0b98b836efdc14153d2872577c54a8bde224500f08635d8a3dae', '0af2a8a06c59ffcd3e166a1641a26ba6ed0cadc9d7a55c025410e9719aac6778', '5fd102f747581ac1502038e199da8eadb9e55fcf339d5e21586d1cd5b8b4c2c6'],
